...
class Efficientdet(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'model_data/efficientdet.pth',
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image, crop = False, *args):
        #---------------------------------------------------#
        #   计算输入图片的高和宽
        #---------------------------------------------------#
        ...
        for i, c in list(enumerate(top_label)):
            ...
            if len(args):
                threeD = args[0]
                # 计算形心
                y = int(top * 2 + bottom * 2) // 4
                x = int(left * 2 + right * 2) // 4
                # if y > 480:y=480
                # if x > 1280:x=1280
                distance = threeD[y][x][-1]
                label = '{} {:.2f} {:.2f}mm'.format(predicted_class,score,distance)
